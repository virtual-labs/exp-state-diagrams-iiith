...
export function getResultRS(ff) {
  ...
  if (
    getOutputRS(ff.r[0], ff.r[1]) !== null &&
    getOutputRS(ff.s[0], ff.s[1]) !== null &&
    getOutputRS(ff.clk[0], ff.clk[1]) !== null
  ) {
    ff.generateOutput();
  }
  return;
...
export class JKFlipFlop {
  ...
  registerComponent(workingArea, x = 0, y = 0) {
    ...
    // Add output Q
    this.addOutputPoints(
      jsPlumbInstance.addEndpoint(el, {
        anchor: [1, 0.3, 1, 0, 7, 1],
        source: true,
        target: true,
        connectionsDetachable: false,
        uuid: "output:2:" + this.id,
        overlays: [
...
export function getResultJK(ff) {
  ...
  if (
    getOutputJK(ff.k[0], ff.k[1]) !== null &&
    getOutputJK(ff.j[0], ff.j[1]) !== null &&
    getOutputJK(ff.clk[0], ff.clk[1]) !== null
  ) {
    ff.generateOutput();
  }
  ...
  for (let key in flipFlops) {
    if (ff.constructor.name === "JKFlipFlop") {
      if (
        ...
        flipFlops[key].j.length !== 0 &&
        flipFlops[key].j[0] === ff
      ) {
        flipFlops[key].j = null;
      }
      if (
        flipFlops[key].k !== null &&
        flipFlops[key].k.length !== 0 &&
        flipFlops[key].k[0] === ff
      ) {
        flipFlops[key].k = null;
      }
      if (
        flipFlops[key].clk !== null &&
        flipFlops[key].clk.length !== 0 &&
        flipFlops[key].clk[0] === ff
      ) {
        flipFlops[key].clk = null;
      }
...
export function deleteJKFlipFlop(id) {
  deleteFF(id);
}
